...
class LoadShiftingEnv(gym.Env):
    ...
    def __init__(self, pv_power, electricity_price, feed_in_tariff, appliance_table):
        ...
        self.Kettle_run_time = appliance_table['Kettle'][0]
        self.Kettle_power = appliance_table['Kettle'][1]

        self.Toaster_run_time = appliance_table['Toaster'][0]
        self.Toaster_power = appliance_table['Toaster'][1]

        self.Vaccum_Cleaner_run_time = appliance_table['Vaccum_Cleaner'][0]
        self.Vaccum_Cleaner_power = appliance_table['Vaccum_Cleaner'][1]

        self.Clothing_Iron_run_time = appliance_table['Clothing_Iron'][0]
        self.Clothing_Iron_power = appliance_table['Clothing_Iron'][1]

        self.Oven_run_time = appliance_table['Oven'][0]
        self.Oven_power = appliance_table['Oven'][1]
        ...
        self.action_space = spaces.MultiDiscrete(
            [self.total_steps - self.Dish_Washer_run_time,
             self.total_steps - self.Kettle_run_time,
             self.total_steps - self.Toaster_run_time,
             self.total_steps - self.Vaccum_Cleaner_run_time,
             self.total_steps - self.Clothing_Iron_run_time,
             self.total_steps - self.Oven_run_time,
             ...
             self.total_steps - self.Washing_machine_run_time])
    ...
    def reset(self):
        ...
        self.Kettle_start = 0
        self.Toaster_start = 0
        self.Vaccum_Cleaner_start = 0
        self.Clothing_Iron_start = 0
        self.Oven_start = 0
        self.EV_start = 0
    ...
    def step(self, action):
        ...
        self.Dish_Washer_start            = action[0] + 1
        self.Kettle_start                 = action[1] + 1
        self.Toaster_start                = action[2] + 1
        self.Vaccum_Cleaner_start         = action[3] + 1
        self.Clothing_Iron_start          = action[4] + 1
        self.Oven_start                   = action[5] + 1
        self.EV_start                     = action[6] + 1
        self.Electric_Water_Heater_start  = action[7] + 1                                                               #######################################################################################
        self.Wash_Dryer_start             = action[8] + 1
        self.Washing_Machine_start        = action[9] + 1
        ...
        for i in range(self.total_steps):
            ...
            # Kettle consumption
            if self.Kettle_start <= i + 1 < self.Kettle_start + self.Kettle_run_time:
                net_power -= self.Kettle_power

            # Toaster consumption
            if self.Toaster_start <= i + 1 < self.Toaster_start + self.Toaster_run_time:
                net_power -= self.Toaster_power

            # Vaccum_Cleaner consumption
            if self.Vaccum_Cleaner_start <= i + 1 < self.Vaccum_Cleaner_start + self.Vaccum_Cleaner_run_time:
                net_power -= self.Vaccum_Cleaner_power

            # Clothing_Iron consumption
            if self.Clothing_Iron_start <= i + 1 < self.Clothing_Iron_start + self.Clothing_Iron_run_time:
                net_power -= self.Clothing_Iron_power

            # Oven consumption
            if self.Oven_start <= i + 1 < self.Oven_start + self.Oven_run_time:
                net_power -= self.Oven_power
