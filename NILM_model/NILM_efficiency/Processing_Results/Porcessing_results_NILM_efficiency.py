...
def Faster_result_processing(csv_path,Actual_Loop_time, Save):
    ...
    df = pd.read_feather(FILE_DIR_PATH/f'{csv_path}/df_NILM_efficiency.feather')
...
#csv_path = f'Results/{NILM_efficiency}/'
#df = Faster_result_processing(csv_path,Actual_Loop_time = 96, Save = True)

#df = pd.read_feather(FILE_DIR_PATH/f'{csv_path}/df_NILM_efficiency.feather')

csv_path_figures = f'Results/{NILM_efficiency}/'
Title = f'NILM efficiency {NILM_efficiency} %'

df = pd.read_feather(FILE_DIR_PATH/f'{csv_path_figures}/df_absolute.feather')

plotting_virtual_HIL(df,csv_path_figures,Title='',Save =True,Show = False)
